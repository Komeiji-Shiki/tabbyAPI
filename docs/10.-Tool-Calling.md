...
With `stream: true`, the Qwen3-Coder/Qwen3.5 parser emits the function name as soon as the opening
function tag arrives. String, object and array parameters are then emitted as incremental
`tool_calls[].function.arguments` JSON fragments while the model is still generating the parameter.
Numeric, boolean and schema-less values remain buffered until their closing parameter tag so their
JSON type can be preserved reliably.
